...
<head>
    ...
    <title>{{ $cashPaymentVoucher->voucher_number }} - Cash Payment Voucher</title>
    <style>
        @page {
            size: A4;
            margin: 0;
...
<body>
    <div class="voucher-container">
        <div class="voucher-header">
            ...
            <div class="voucher-type">Voucher No: {{ $cashPaymentVoucher->voucher_number }}</div>
        </div>

        <div class="voucher-info">
            <div class="info-item">
                <span class="info-label">Date:</span>
                <span>{{ $cashPaymentVoucher->payment_date->format('d/m/Y') }}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Cash Account:</span>
                <span>{{ optional($cashPaymentVoucher->cashAccount)->account_code ?? '—' }}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Account Name:</span>
                <span>{{ optional($cashPaymentVoucher->cashAccount)->account_name ?? '—' }}</span>
            </div>
            @if($cashPaymentVoucher->reference_number)
            <div class="info-item">
                <span class="info-label">Reference:</span>
                <span>{{ $cashPaymentVoucher->reference_number }}</span>
            </div>
            @endif
        </div>
        ...
        <table class="voucher-table">
            ...
            <tbody>
                ...
                <tr class="total-row">
                    ...
                    <td class="amount">{{ number_format($cashPaymentVoucher->amount, 2) }}</td>
                </tr>
            </tbody>
        </table>

        @if($cashPaymentVoucher->notes)
        <div class="notes-section">
            <strong>Notes:</strong>
            <div>{{ $cashPaymentVoucher->notes }}</div>
        </div>
        @endif
