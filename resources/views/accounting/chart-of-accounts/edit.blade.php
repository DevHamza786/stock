            ...
            <div class="mb-8">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="mt-2 text-gray-600">Update account information for {{ $chartOfAccount->account_name }}</p>
                    </div>
                </div>
            </div>
                    ...
                    <form method="POST" action="{{ route('accounting.chart-of-accounts.update', $chartOfAccount) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" id="account_code" name="account_code" value="{{ old('account_code', $chartOfAccount->account_code) }}" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('account_code') border-red-500 @enderror" placeholder="e.g., 1130" required>
                                @error('account_code')
                            ...
                            <div>
                                ...
                                <input type="text" id="account_name" name="account_name" value="{{ old('account_name', $chartOfAccount->account_name) }}" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('account_name') border-red-500 @enderror" placeholder="e.g., Particulars Inventory" required>
                                @error('account_name')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <select id="account_subtype" name="account_subtype" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('account_subtype') border-red-500 @enderror" required>
                                    ...
                                    @foreach($accountSubtypes as $key => $value)
                                        <option value="{{ $key }}" {{ old('account_subtype', $chartOfAccount->account_subtype) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="level" name="level" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('level') border-red-500 @enderror" required>
                                    ...
                                    <option value="1" {{ old('level', $chartOfAccount->level) == '1' ? 'selected' : '' }}>Level 1 (Main Account)</option>
                                    <option value="2" {{ old('level', $chartOfAccount->level) == '2' ? 'selected' : '' }}>Level 2 (Sub Account)</option>
                                    <option value="3" {{ old('level', $chartOfAccount->level) == '3' ? 'selected' : '' }}>Level 3 (Detail Account)</option>
                                    <option value="4" {{ old('level', $chartOfAccount->level) == '4' ? 'selected' : '' }}>Level 4 (Sub Detail)</option>
                                    <option value="5" {{ old('level', $chartOfAccount->level) == '5' ? 'selected' : '' }}>Level 5 (Final Detail)</option>
                                </select>
                                @error('level')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" id="opening_balance" name="opening_balance" value="{{ old('opening_balance', $chartOfAccount->opening_balance) }}" step="0.01" min="0" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('opening_balance') border-red-500 @enderror" required>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="description" name="description" rows="3" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror" placeholder="Enter account description...">{{ old('description', $chartOfAccount->description) }}</textarea>
                                @error('description')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                <div class="flex items-center space-x-6">
                                    <div class="flex items-center">
                                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $chartOfAccount->is_active) ? 'checked' : '' }} class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                                        <label for="is_active" class="ml-2 block text-sm text-gray-900">Active Account</label>
                                    </div>
                                    <div class="flex items-center">
                                        <input type="checkbox" id="is_system_account" name="is_system_account" value="1" {{ old('is_system_account', $chartOfAccount->is_system_account) ? 'checked' : '' }} class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
