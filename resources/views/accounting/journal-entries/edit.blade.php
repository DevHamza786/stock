<x-app-layout>
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">Edit Journal Entry</h1>
                        <p class="mt-2 text-gray-600">Update the details of this accounting transaction</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-lg rounded-xl border border-gray-200">
                <div class="p-6">
                    <form method="POST" action="{{ route('accounting.journal-entries.update', $journalEntry) }}" id="journal-entry-form">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                            <div>
                                ...
                                <input
                                    ...
                                    class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('entry_date') border-red-500 @enderror"
                                    required
                                >
                                @error('entry_date')
                            ...
                            <div>
                                ...
                                <select
                                    id="entry_type"
                                    name="entry_type"
                                    class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('entry_type') border-red-500 @enderror"
                                    required
                                    {{ $journalEntry->status === 'POSTED' ? 'disabled' : '' }}
                                >
                                    <option value="">Select entry type...</option>
                                    @foreach($entryTypes as $key => $value)
                                        <option value="{{ $key }}" {{ old('entry_type', $journalEntry->entry_type) == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <div id="balance-check" class="px-3 py-2 border border-gray-300 rounded-lg bg-gray-50">
                                    <span class="text-sm text-gray-500">Transactions will update balance information</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea
                                ...
                                required
                            >{{ old('description', $journalEntry->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            >{{ old('notes', $journalEntry->notes) }}</textarea>
                            @error('notes')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex justify-end space-x-4">
                            <a href="{{ route('accounting.journal-entries.show', $journalEntry) }}" class="px-6 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-lg transition-colors duration-200">
                                Cancel
                            </a>
    ...
    <script>
        ...
        const existingTransactions = @json(old('transactions', $transactionFormData));
        ...
        function addTransaction(transaction = null) {
            ...
            const transactionHtml = `
                <div class="transaction-row border border-gray-200 rounded-lg p-4 mb-4" data-index="${index}">
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Account *</label>
                            <select name="transactions[${index}][account_id]" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                                <option value="">Select account...</option>
                                ${accounts.map(account => `<option value="${account.id}" ${String(data.account_id) === String(account.id) ? 'selected' : ''}>${escapeHtml(account.account_code)} - ${escapeHtml(account.account_name)}</option>`).join('')}
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Description *</label>
                            <input type="text" name="transactions[${index}][description]" value="${escapeHtml(formatValue(data.description))}" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Transaction description..." required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Debit Amount</label>
                            <input type="number" name="transactions[${index}][debit_amount]" step="0.01" min="0" value="${escapeHtml(formatValue(data.debit_amount))}" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent debit-amount" placeholder="0.00">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Credit Amount</label>
                            <input type="number" name="transactions[${index}][credit_amount]" step="0.01" min="0" value="${escapeHtml(formatValue(data.credit_amount))}" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent credit-amount" placeholder="0.00">
                        </div>
        ...
        if (existingTransactions && existingTransactions.length > 0) {
            existingTransactions.forEach(transaction => addTransaction(transaction));
        } else {
            addTransaction();
        }
    </script>
