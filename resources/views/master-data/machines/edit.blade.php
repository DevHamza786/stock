        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('master-data.machines.update', $machine) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select id="status" name="status" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('status') border-red-500 @enderror">
                                    <option value="1" {{ old('status', $machine->status) == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ old('status', $machine->status) == '0' ? 'selected' : '' }}>Inactive</option>
                                </select>
                                @error('status')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
