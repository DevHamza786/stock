<x-app-layout>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Handle existing data conversion from 3D format if needed
            function convertExistingData() {
                // Check if we have existing data in the old format and need to convert
                // This would be handled by the backend, but we can add frontend logic if needed
                const existingLength = lengthInput.value;
                const existingHeight = heightInput.value;
                
                // If we have existing data, calculate immediately
                if (existingLength && existingHeight) {
                    calculateSize();
                }
            }
